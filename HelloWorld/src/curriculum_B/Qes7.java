...
public class Qes7 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < person; i++) {
			// 変数subave2にline配列のline[i][1]を加算
			subave2 += line[i][1];
		}
		System.out.println("数学の平均点は" + subave2 / person + "点です");

		for (int i = 0; i < person; i++) {
			// 変数subave3にline配列のline[i][2]を加算
			subave3 += line[i][2];
		}
		System.out.println("理科の平均点は" + subave3 / person + "点です");

		for (int i = 0; i < person; i++) {
			// 変数subave4にline配列のline[i][3]を加算
			subave4 += line[i][3];
		}
		System.out.println("社会の平均点は" + subave4 / person + "点です");
		// sysoutで上記で変数に代入していた各教科の平均点を合計し数でわった全体の平均をsysoutで出力
		System.out.println(
				"全体の平均点は" + (subave1 / person + subave2 / person + subave3 / person + subave4 / person) / 4 + "です。");
